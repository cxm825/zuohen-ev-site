...
function enrich(product, id) {
  ...
  const productCategory = product.category || 'EV Charging';
  const power = product.power || (text.match(/\b(?:\d{1,3}(?:\.\d+)?\s?kw|\d{2,3}a|\d{3,4}v)\b/gi) || []).slice(0, 3).join(' · ');
  const price = 'Request quotation';
  const moq = 'Confirm with sales';
  return { ...product, id: product.id ?? id, category: productCategory, power, price, moq, displayTitle: product.displayTitle || product.title };
}

function card(product) {
  const image = product.image && product.image.startsWith('assets/') ? product.image : '';
  const safeTitle = (product.displayTitle || product.title).replace(/"/g, '&quot;');
